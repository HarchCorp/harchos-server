...
def _estimate_inference_carbon(
    ...
    """
    gpu_power_w = {"H100": 700, "A100": 400, "L40S": 350}.get(gpu_type, 500)
...
async def chat_completions(
    ...
    if request.model not in model_ids:
        raise HTTPException(
            status_code=400,
            detail=f"Model '{request.model}' not found. Available: {', '.join(model_ids[:5])}..."
        )
    ...
    # In production, this would call vLLM/Together API
    # For now, generate a high-quality mock response
    messages_content = [m.content for m in request.messages if m.role == "user"]
    user_msg = messages_content[-1] if messages_content else ""
    
    # Simulate processing delay
    await asyncio.sleep(0.05)
    
    prompt_tokens = sum(len(m.content.split()) for m in request.messages) + len(request.messages)
    ...
    response_text = f"I'm running on HarchOS infrastructure in {hub_region}, Morocco — powered by {intensity.renewable_percentage:.0f}% renewable energy with a carbon intensity of just {intensity.carbon_intensity_gco2_kwh:.0f} gCO2/kWh. This inference generated approximately {carbon.gco2_per_request:.4f} grams of CO2. No other AI platform gives you this level of carbon transparency. How can I help you today?"
...
async def completions(
    ...
    return CompletionResponse(
        ...
        choices=[
            CompletionChoice(
                index=0,
                text=f"[HarchOS Carbon-Aware Inference] Processed on {intensity.renewable_percentage:.0f}% renewable energy. CO2: {carbon.gco2_per_request:.4f}g. ",
                finish_reason="stop",
            )
        ],
        usage=UsageInfo(
...
async def _stream_chat_completion(
    ...
    words = f"I'm running on HarchOS in {hub_region} — {intensity.renewable_percentage:.0f}% renewable, {intensity.carbon_intensity_gco2_kwh:.0f} gCO2/kWh. This inference: {carbon.gco2_per_request:.4f}g CO2. No other platform offers carbon transparency like this.".split()
